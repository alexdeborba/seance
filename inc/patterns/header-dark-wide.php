<?php
/**
 * Séance: Large header with dark background block pattern
 */
return array(
	'title'      => __( 'Large header with dark background', 'seance' ),
	...
	'content'    => '<!-- wp:group {"align":"full","style":{"elements":{"link":{"color":{"text":"var:preset|color|background"}}},"spacing":{"padding":{"top":"0px","bottom":"var(--wp--custom--spacing--large, 8rem)"}}},"backgroundColor":"foreground","textColor":"background","layout":{"inherit":true}} -->
					...
					<h2 class="alignwide" style="font-size:clamp(3.25rem, 8vw, 6.25rem);line-height:1.15">' . wp_kses_post( __( '<em>Séance</em>: A Theme Born Out of Love and Death...', 'seance' ) ) . '</h2>
					<!-- /wp:heading --></div>
					<!-- /wp:group -->

					<!-- wp:image {"align":"full","sizeSlug":"full","linkDestination":"none"} -->
					<figure class="wp-block-image alignfull size-full"><img src="' . esc_url( get_template_directory_uri() ) . '/assets/images/thumbnail-path-on-transparent-c.png" alt="' . esc_attr__( 'Illustration.', 'seance' ) . '"/></figure>
					<!-- /wp:image --></div>
